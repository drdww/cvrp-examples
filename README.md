...
Example 3 writes `output/03_hartford_routes.html` â€” open it in a browser and
...
3. **Distance matrix**: one Dijkstra per location (101 total) gives a
   101Ã—101 matrix of *road* travel times. It is asymmetric â€” Aâ†’B â‰  Bâ†’A â€”
...
This is exactly the architecture of a production routing stack â€” in industry
you'd swap step 1â€“3 for a routing engine like **OSRM**, **Valhalla**, or the
Google Distance Matrix API (faster matrices, live traffic), and keep
steps 4â€“5 unchanged.

## âš ï¸ Node routing vs. arc routing (important for "clearing roads")
...
- **Capacitated Arc Routing Problem (CARP)** â€” the snow-plow / street-sweeper
...
and then feed it to OR-Tools â€” that's a great "advanced" lecture. The
...
2. **Radial feeders**: a shortest-path forest grown from the substations â€”
   every pole is fed by its nearest substation along the streets. Radial
   topology (power flows one way) falls out automatically.
3. **Backbone vs. lateral**: tree spans serving â‰¥ 800 downstream customers
...
- **Objective**: utilities don't minimize miles â€” they minimize
  **customer-minutes of interruption** (CMI; SAIDI is CMI Ã· customers).
...
- **Weights**: each damage is credited with the customers whose *nearest
  upstream* damage it is â€” one feeder span can be worth 1,200 customers,
  a lateral 8.
...
1. **Damage assessment first** â€” you can't optimize what you haven't
...
4. **Rolling horizon**: re-solve every 1â€“2 hours as new damage reports
...
The experiment proposed in [SCALING.md](SCALING.md) Â§6. Same mock grid as
...
- **(a) static** â€” solve once at t=0, follow the plan, take late
  discoveries first-come-first-served. Consistently **14â€“26% worse CMI**
  across seeds: the cost of ignoring current information.
- **(b) greedy** â€” each freed crew takes the best customers-per-hour job.
- **(c) rolling horizon** â€” each freed crew re-solves the weighted-latency
  problem over all revealed unfinished jobs and takes the first step.

Honest finding: greedy often *ties* rolling at this scale â€” with 5 crews
...
Output: `output/05_policy_comparison.png` â€” three outage curves on one
axis.

## Roadmap: beyond OR-Tools

### Stage 2 â€” specialized metaheuristic solvers (usually beat OR-Tools on pure CVRP)

| Solver | Notes |
|--------|-------|
| **PyVRP** (`pip install pyvrp`) | State-of-the-art Hybrid Genetic Search (HGS), pure Python API, actively maintained, wins DIMACS/VRP competitions. Easiest next step â€” same matrix in, routes out. |
...
### Stage 3 â€” exact / mathematical programming

| Approach | Notes |
|----------|-------|
| **MILP (Gurobi / HiGHS / CP-SAT)** | Two/three-index flow formulations with MTZ or subtour-elimination cuts. Proves optimality but only to ~50â€“80 customers. Great for teaching duality/bounds. |
| **Branch-cut-and-price (VRPSolver)** | The academic state of the art for exact CVRP (hundreds of customers). |

A natural lecture: run PyVRP and a MILP on the same 30-customer instance â€”
the metaheuristic finds the optimum in seconds, the MILP *proves* it.

### Stage 4 â€” quantum and quantum-inspired
...
1. **QUBO formulation**: binary variable x[v,i,t] = "vehicle v is at location
   ...
   penalties. Watch qubit counts explode: even 20 locations â†’ thousands of
   binary variables. This *is* the lesson.
2. **Quantum annealing â€” D-Wave** (`pip install dwave-ocean-sdk`): hybrid
...
3. **Gate-model â€” QAOA via Qiskit** (`pip install qiskit qiskit-optimization`):
   solve a 4â€“5 location toy VRP on a simulator; demonstrates the variational
...
4. **Quantum-inspired classical**: simulated-annealing / simulated-bifurcation
   QUBO samplers (`neal`, Fujitsu Digital Annealer style) â€” good baseline
   showing the QUBO *formulation* can be attacked classically too.

Suggested arc for a course module:
OR-Tools (this repo) â†’ PyVRP on the same Hartford matrix â†’ MILP bound on a
subsample â†’ QUBO on a 5-stop toy â†’ discussion of where quantum could matter.
